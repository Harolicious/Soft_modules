...
x1=0
x2=7
x = np.linspace(x1, x2, len(mate_desp))
y = m * x + b 

diff = np.array(mate_desp - y)
curva_aju = y - diff
...
pruebas_df_opt = dividir_y_extraer_columna(df_opt, tamanos_opt, 6)  # Desp
pruebas_df_ard = dividir_y_extraer_columna(df_ard, tamanos_ard, 1)  # PSI
pruebas_df_opt2 = dividir_y_extraer_columna(df_opt, tamanos_opt, 5)  # Def
pruebas_df_ard2 = dividir_y_extraer_columna(df_ard, tamanos_ard, 1)  # PSI

# Seleccion de las pruebas
pruebas_df_opt = pruebas_df_opt[1:9]
pruebas_df_ard = pruebas_df_ard[1:9]
pruebas_df_opt2 = pruebas_df_opt2[1:9]
pruebas_df_ard2 = pruebas_df_ard2[1:9]
...
window_length = 15  # Ajusta el tamaño de la ventana para el suavizado
...
errores_pres_opt2_smooth = savgol_filter(errores_pres_opt2, window_length, polyorder)
errores_desp_opt2_smooth = savgol_filter(errores_desp_opt2, window_length, polyorder)
errores_pres_ard2_smooth = savgol_filter(errores_pres_ard2, window_length, polyorder)
errores_desp_ard2_smooth = savgol_filter(errores_desp_ard2, window_length, polyorder)

# Ajustar los promedios suavizados para que solo sean positivos
promedios_pres_opt_smooth = np.maximum(promedios_pres_opt_smooth, 0)
promedios_desp_opt_smooth = np.maximum(promedios_desp_opt_smooth, 0)
promedios_pres_ard_smooth = np.maximum(promedios_pres_ard_smooth, 0)
promedios_desp_ard_smooth = np.maximum(promedios_desp_ard_smooth, 0)

# promedios_pres_opt2_smooth = np.maximum(promedios_pres_opt2_smooth, 0)
# promedios_desp_opt2_smooth = np.maximum(promedios_desp_opt2_smooth, 0)
# promedios_pres_ard2_smooth = np.maximum(promedios_pres_ard2_smooth, 0)
# promedios_desp_ard2_smooth = np.maximum(promedios_desp_ard2_smooth, 0)

mate_PSI = np.array(mate_PSI)
mate_desp = np.array(mate_desp)

def graficar_promedio_con_error(promedios_pres_ard, promedios_pres_opt, errores_pres_opt_smooth, promedios_desp_ard, promedios_desp_opt, errores_desp_opt_smooth):
    plt.figure(figsize=(10,6))

    plt.plot(promedios_pres_ard_smooth, promedios_pres_opt_smooth, marker=',', color='b', label='Average Compresion Inflated')
    plt.fill_between(promedios_pres_ard_smooth, promedios_pres_opt_smooth - errores_pres_opt_smooth, promedios_pres_opt_smooth + errores_pres_opt_smooth, color='b', alpha=0.3)
  
    plt.plot(promedios_desp_ard_smooth, promedios_desp_opt_smooth, marker=',', color='r', label='Average Compresion Deflated')
    plt.fill_between(promedios_desp_ard, promedios_desp_opt - errores_desp_opt_smooth, promedios_desp_opt + errores_desp_opt_smooth, color='r', alpha=0.3)
 
    plt.plot(mate_PSI, mate_desp, marker=',', color='black', label='FEM Mooney-Rivlin')
    # plt.plot(x,y)
    # plt.plot(x,diff)
    # plt.plot(x,curva_aju)
    plt.title('Pressure vs Displacement Z - Robot Uniaxial', size = 20)
    plt.xlabel('Pressure (PSI)', size = 20)
    plt.ylabel('Displacement Z (mm)', size = 20)
    # plt.axis('equal')
    plt.grid(True)
    plt.legend()
    plt.savefig("SE_of_Strain_wanted_Estirar.pdf", format="pdf", bbox_inches="tight", dpi=300)
...
def graficar_promedio_con_error(promedios_pres_ard, promedios_pres_opt, errores_pres_opt_smooth, promedios_desp_ard, promedios_desp_opt, errores_desp_opt_smooth):
    ...
    plt.title('Pressure vs Displacement Z - Robot Uniaxial')
    plt.xlabel('Pressure (PSI)')
    plt.ylabel('Displacement Z (mm)')
    # plt.axis('equal')
    # plt.grid(True)
    plt.legend()
    plt.savefig("SE_of_Strain_wanted_NoFEM_Estirar.pdf", format="pdf", bbox_inches="tight", dpi=300)

def graficar_promedio_con_error_2(promedios_pres_ard2_smooth, promedios_pres_opt2_smooth, errores_pres_opt2_smooth, promedios_desp_ard2_smooth, promedios_desp_opt2_smooth, errores_desp_opt2_smooth):
    plt.figure(figsize=(10,6))

    plt.plot(promedios_pres_ard2_smooth, promedios_pres_opt2_smooth, marker=',', color='b', label='Average Compresion Inflated')
    plt.fill_between(promedios_pres_ard2_smooth, promedios_pres_opt2_smooth - errores_pres_opt2, promedios_pres_opt2_smooth + errores_pres_opt2_smooth, color='b', alpha=0.3)
   
    plt.plot(promedios_desp_ard2_smooth, promedios_desp_opt2_smooth, marker=',', color='r', label='Average Compresion Deflated')
    plt.fill_between(promedios_desp_ard2_smooth, promedios_desp_opt2_smooth - errores_desp_opt2_smooth, promedios_desp_opt2_smooth + errores_desp_opt2_smooth, color='r', alpha=0.3)
    plt.title('Pressure vs Diameter - Robot Uniaxial')
    plt.xlabel('Pressure (PSI)')
    plt.ylabel('Displacement X (mm)')
    # plt.axis('equal')
    # plt.grid(True)
    plt.legend()
    plt.savefig("SE_of_Strain_unwanted_Estirar.pdf", format="pdf", bbox_inches="tight", dpi=300)


graficar_promedio_con_error(promedios_pres_ard_smooth, promedios_pres_opt_smooth, errores_pres_opt_smooth, promedios_desp_ard_smooth, promedios_desp_opt_smooth, errores_desp_opt_smooth)
graficar_promedio_con_error_2(promedios_pres_ard2_smooth, promedios_pres_opt2_smooth, errores_pres_opt2_smooth, promedios_desp_ard2_smooth, promedios_desp_opt2_smooth, errores_desp_opt2_smooth)
