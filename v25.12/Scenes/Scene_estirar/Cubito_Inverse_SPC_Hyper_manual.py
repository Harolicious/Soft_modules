...
class Controller(Sofa.Core.Controller): 
    def __init__(self, *args, **kwargs): 
        ...
        self.YM_values = [3000, 6000, 9000, 12000]
        self.pressure_ranges = [0, 15, 30, 40, 50]
        
        self.csv_file_path = "end_effector_data_Estirar_SPC.csv" 
    ...
    def update_pressure_increase(self, pressure, SPC): 
        ...
        SPC.pressure.value = float(pressure)
        return pressure 
    ...
    def update_young_modulus(self):
        for i in range(len(self.pressure_ranges) - 1):
            if self.pressure_ranges[i] <= self.Pressure < self.pressure_ranges[i + 1]:
                ...
                alpha = 0.1
                new_YM = current_YM * (1 - alpha) + target_YM * alpha
                self.FEM_main.youngModulus.value = float(new_YM)
                return
  
    def onAnimateBeginEvent(self, eventType): 
        
        current_time = self.RootNode.time.value 
        print(f"Pressure: {self.Pressure:.2f} | YM: {float(self.FEM_main.youngModulus.value):.2f} | SPC: {float(self.SPC.pressure.value):.2f}")
...
def createScene(rootNode):
        ...
        #cubito
                ...
                cubito.addObject('ShewchukPCGLinearSolver', iterations=50, name='linearsolver', tolerance=1e-5, preconditioner='@preconditioner', use_precond=True, update_step=1)
        ...
        #cubito/cavity
                ...
                SPC = cavity.addObject('SurfacePressureConstraint',
        ...
        #cubito/visu
                ...
                rootNode.addObject(Controller(name="ActuationController", RootNode=rootNode, SPC=SPC, EndEffectorMO=EndEffectorMO, EndEffectorMO2=EndEffectorMO2, FEM_main=FEM_main))
