...
class Controller(Sofa.Core.Controller): 
    def __init__(self, *args, **kwargs): 
        ...
        # 10 tramos de 5 kPa — YM ajustados desde datos FEM (R²>0.99)
        self.pressure_ranges = [0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0]
        
        # [0.0, 2.5, 5.0, 7.5, 10.0, 12.5, 15.0, 17.5, 20.0, 22.5, 25.0, 27.5, 30.0, 32.5, 35.0, 37.5, 40.0, 42.5, 45.0, 47.5, 50.0]
        
        self.YM_values       = [24000, 20000, 16000, 14000, 13000, 11000, 10000, 9500, 9000, 8500]
        
        # [32000, 28000, 25000, 23000, 21000, 19000, 17500, 16000, 14500, 13000, 12000, 11000, 10500, 10000, 9750, 9500, 9250, 9000, 8750, 8500]
               
        self.SPA_maxPressure_values = [5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0]
        ...
        self.SPA.maxPressure.value = np.float64(self.SPA_maxPressure_values[0])

        self.csv_file_path = "end_effector_data_Estirar_inverso.csv" 
        if not os.path.exists(self.csv_file_path): 
            with open(self.csv_file_path, mode='w', newline='') as file: 
                writer = csv.writer(file) 
                writer.writerow(["Time", "Goal_Desp", "SPA_Pressure_calculated", "YM",
    ...
    def save_data(self, time):
        ...
        try:
            with open(self.csv_file_path, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([time, self.GoalDesp, self.get_spa_pressure(), self.get_ym(),
                                  p1[0][0], p1[0][1], p1[0][2],
                                  p2[0][0], p2[0][1], p2[0][2]])
        except Exception as e:
            print(f"Error al escribir CSV: {e}")

    def pressure_equiv(self):
        return (self.GoalDesp / self.MaxDesp) * 50.0

    def update_young_modulus(self):
        # Usar GoalDesp para seleccionar el tramo
        # GoalDesp avanza uniformemente → garantiza que todos los tramos se recorran
    ...
    def update_max_pressure(self):
        # maxPressure sube de forma uniforme con el GoalDesp
        # 0 mm → 0 kPa,  7.67 mm → 50 kPa  (escala lineal)
        p_equiv = (self.GoalDesp / self.MaxDesp) * 50.0
        p_equiv = max(1.0, p_equiv)
        # P_real = lambda × 10  →  maxPressure = p_equiv / 10
