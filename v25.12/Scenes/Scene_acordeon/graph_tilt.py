...
df_opt = pd.read_csv('acordeon_datos_opt.csv')
df_ard = pd.read_csv('acordeon_datos_ard.csv')
# df_opt2 = pd.read_csv('Uniaxial_d_opt.csv')
# df_ard2 = pd.read_csv('Uniaxial_d_ard.csv')
mate = pd.read_excel('acordeon.xlsx')
...
# df_opt.iloc[:, 4] = df_opt.iloc[:, 4] * 1.2     #? 
# df_ard.iloc[:, 1] = df_ard.iloc[:, 1] * 1.28    #?
...
pruebas_df_opt2 = dividir_y_extraer_columna(df_opt, tamanos_opt, 4)  # Angle
...
def graficar_promedio_con_error(promedios_pres_ard_smooth, promedios_pres_opt_smooth, errores_pres_opt_smooth, promedios_desp_ard_smooth, promedios_desp_opt_smooth, errores_desp_opt_smooth):
    ...
    plt.fill_between(promedios_desp_ard_smooth, promedios_desp_opt_smooth - errores_desp_opt_smooth, promedios_desp_opt_smooth + errores_desp_opt_smooth, color='r', alpha=0.3)
 
    # plt.scatter(mate_PSI, mate_desp, marker='.', color='black', label='FEM Mooney-Rivlin')
    plt.title('Pressure vs Displacement Z - Robot Tilt')
    plt.xlabel('Pressure (PSI)')
    plt.ylabel('Displacement Z (mm)')
    # plt.axis('equal')
    # plt.grid(True)
    plt.legend()
    plt.savefig("SE_of_Strain_unwanted_acordeon.pdf", format="pdf", bbox_inches="tight", dpi=300)

def graficar_promedio_con_error_2(promedios_pres_ard2, promedios_pres_opt2, errores_pres_opt2, promedios_desp_ard2, promedios_desp_opt2, errores_desp_opt2):
    plt.figure(figsize=(10,6))

    plt.plot(promedios_pres_ard2_smooth, promedios_pres_opt2_smooth, marker=',', color='b', label='Average Angle Inflated')
    plt.fill_between(promedios_pres_ard2, promedios_pres_opt2 - errores_pres_opt2, promedios_pres_opt2 + errores_pres_opt2, color='b', alpha=0.3)
   
    plt.plot(promedios_desp_ard2_smooth, promedios_desp_opt2_smooth, marker=',', color='r', label='Average Anglebender Deflated')
    plt.fill_between(promedios_desp_ard2, promedios_desp_opt2 - errores_desp_opt2, promedios_desp_opt2 + errores_desp_opt2, color='r', alpha=0.3)
    
    plt.plot(mate_PSI, mate_desp, marker=',', color='black', label='FEM Mooney-Rivlin')
    
    plt.title('Pressure vs Angle - Robot Tilt', size = 20)
    plt.xlabel('Pressure (PSI)', size = 20)
    plt.ylabel('Angle (Degree)', size = 20)
    # plt.axis('equal')
    plt.grid(True)
    plt.legend()
    plt.savefig("SE_of_Strain_wanted_acordeon.pdf", format="pdf", bbox_inches="tight", dpi=300)
...
def graficar_promedio_con_error_2(promedios_pres_ard2, promedios_pres_opt2, errores_pres_opt2, promedios_desp_ard2, promedios_desp_opt2, errores_desp_opt2):
    ...
    plt.title('Pressure vs Angle - Robot Tilt')
    plt.xlabel('Pressure (PSI)')
    plt.ylabel('Angle (Degree)')
    # plt.axis('equal')
    # plt.grid(True)
    plt.legend()
    plt.savefig("SE_of_Strain_wanted_NoFEM_acordeon.pdf", format="pdf", bbox_inches="tight", dpi=300)
