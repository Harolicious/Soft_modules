...
files = [f'end_effector_data_Rotador_NoC_{i}.csv' for i in range(2, 15)]
...
for idx, file in enumerate(files):
    ...
    plt.plot(Pressure_PSI, Angle, color=color, label=f'Cantidad de fibras = {i}')
...
plt.grid(True)
plt.tight_layout()
plt.savefig('Angle_vs_Pressure_NoC_Rot.png', dpi=300)
plt.show()
...
for idx, file in enumerate(files):
    ...
    plt.plot(Pressure_PSI, Position_Y, color=color, label=f'Cantidad de fibras = {i}')
...
plt.grid(True)
plt.tight_layout()
plt.savefig('UW_Displacement_vs_Pressure_NoC_Rot.png', dpi=300)
plt.show()
