...
class Controller(Sofa.Core.Controller):   
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.csv_file_path = f"end_effector_data_Rotador_No_Deg_{Deg}.csv"
...
def createScene(rootNode):
        ...
        #cubito/fibers
                ...
                deltaAngle = 2 * np.pi / numFibers  
                
                Points = []
                Edges = []
                
                for fiber in range(numFibers):
                    baseAngle = fiber * deltaAngle 
                    for i in range(Density):
           
                        Angle = baseAngle + i * deltaHeight / Radius * np.tan(Rad) 
                        Coords = [
                            Radius * np.cos(Angle),  
                            H + i * deltaHeight,    
                            Radius * np.sin(Angle)   
