...
files = [f'end_effector_data_Estirar_NoC_{i}.csv' for i in range(3, 17)]
...
plt.title('Relación entre desplazamiento en Z y presión')
plt.grid(True)
plt.tight_layout()  # Ajustar layout para evitar solapamientos
plt.savefig('Displacement_Pressure_NoC.png', dpi=300)
plt.show()
...
plt.xlabel('Pressure (PSI)')
plt.title('Relación entre el desplazamiento en Y y presión')
plt.grid(True)
plt.tight_layout()  # Ajustar layout para evitar solapamientos
plt.savefig('UW_Displacement_Pressure_NoC_Y.png', dpi=300)
plt.show()
