...
files = [f'end_effector_data_Estirar_NoRad_{i}.csv' for i in range(2, 10)]
...
for idx, file in enumerate(files):
    i = idx + 2  # Número del archivo (1 a 9)
    ...
    plt.plot(Pressure_PSI, y1_ad, color=color, marker=',', label=f'Radio del Núcleo = {i}')
...
plt.savefig('Displacement_Pressure_NoRad.png', dpi=300)
plt.show()
...
plt.grid(True)
plt.tight_layout()  # Ajustar layout para evitar solapamientos
plt.savefig('UW_Displacement_Pressure_NoRad.png', dpi=300)
plt.show()
