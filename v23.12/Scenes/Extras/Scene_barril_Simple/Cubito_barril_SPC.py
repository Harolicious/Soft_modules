...
def createScene(rootNode):
        ...
        #cubito/fibers
                ...
                for i in range(Density):
                    y = Ymin + i * dy   # 🔥 largo fijo de fibra
                    ...
                    for j in range(Repeat):
                        ...
                        # Fibra vertical (independiente)
                        if i < Density - 1:
                            idx_up = (i + 1) * Repeat + j
                            Edges.append([idx, idx_up])
                
                        # Conexión opuesta solo en tapas
                        if (i == 0 or i == Density - 1) and j < half:
                            idx_opposite = i*Repeat + (j + half)
                            Edges.append([idx, idx_opposite])
