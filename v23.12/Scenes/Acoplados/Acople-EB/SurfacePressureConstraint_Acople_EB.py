...
class Controller(Sofa.Core.Controller):   
    ...
    def onAnimateBeginEvent(self, eventType):
        ...
        if self.Pressure > 50 or self.Pressure < 0:
            # self.Pressure = 550
...
def createScene(rootNode):
		...
		#cubito1
                ...
                boxROIStiffness = cubito.addObject('BoxROI', name='boxROIStiffness', box=[-13, 37.5, -13,  13, 41, 13], drawBoxes=True, position="@tetras.rest_position", tetrahedra="@container.tetrahedra")
...
##############Estirar
                ...
                for i in range(NLevels):
                    for j in range(0,30): 
                        ...
                        Coords = [Radius*np.cos(Angle), 20 + 4 + i*LevelHeight, Radius*np.sin(Angle)]
                        Points.append(Coords)
                        if j>=1:
